"""
pure_python_file_2024_tw_posts_president_scored_anon.py
...
Output is written to 'pure_python_output_2024_tw_posts_president_scored_anon.txt'.
"""
import csv
...
if __name__ == "__main__":
    ...
    output_path = r'C:\Users\amank\OneDrive - Syracuse University\Syracuse Master Folder\Research Analyst Project for OPT\pure_python_output_2024_tw_posts_president_scored_anon.txt'
    with open(output_path, 'w', encoding='utf-8') as f:
        ...
        groups_source = group_by(raw_data, ['source'])
        print('\n===== Grouped by source (showing first 3 groups) =====')
        for group_key, group_data in list(groups_source.items())[:3]:
            describe_data(header, group_data, numeric_cols, group_name=f'source={group_key[0]}')

        groups_source_id = group_by(raw_data, ['source', 'id'])
        print('\n===== Grouped by source and id (showing first 3 groups) =====')
        for group_key, group_data in list(groups_source_id.items())[:3]:
            describe_data(header, group_data, numeric_cols, group_name=f'source={group_key[0]}, id={group_key[1]}')
